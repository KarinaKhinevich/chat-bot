...
class TestIntegrationWorkflow:
    ...
    def test_document_upload_and_list_workflow(
        ...
        mock_summarizer_instance.summarize_document.return_value = "Integration test summary"
        ...
        files = {"file": ("integration_test.txt", io.BytesIO(sample_txt_content), "text/plain")}
    ...
    def test_document_upload_summary_and_delete_workflow(
        ...
        mock_summarizer_instance = AsyncMock()
        mock_summarizer.return_value = mock_summarizer_instance
        mock_summarizer_instance.summarize_document.return_value = "This is a test summary"

        mock_pg_service_instance = AsyncMock()
        mock_pg_service.return_value = mock_pg_service_instance
        mock_pg_service_instance.create_document.return_value = None
        mock_pg_service_instance.delete_document_by_metadata.return_value = 3  # 3 chunks deleted

        # Step 1: Upload a document
        files = {"file": ("summary_test.txt", io.BytesIO(sample_txt_content), "text/plain")}
        upload_response = client.post("/document", files=files)
