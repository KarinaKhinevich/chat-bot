...
class TestPGDocumentService:
    ...
    async def test_create_document_success(self):
        ...
        with patch('chat_bot.services.pg_document_service.OpenAIEmbeddings') as mock_embeddings, \
             patch('chat_bot.services.pg_document_service.DocumentChunker') as mock_chunker_class, \
             patch.object(PGDocumentService, 'init_pgvector') as mock_init_pgvector:
    ...
    async def test_delete_document_by_metadata_success(self):
        ...
        with patch('chat_bot.services.pg_document_service.OpenAIEmbeddings') as mock_embeddings, \
             patch('chat_bot.services.pg_document_service.DocumentChunker') as mock_chunker_class, \
             patch('chat_bot.services.pg_document_service.engine') as mock_db_engine:
    ...
    async def test_create_document_empty_content(self):
        ...
        with patch('chat_bot.services.pg_document_service.OpenAIEmbeddings') as mock_embeddings, \
            ...
            mock_chunker.index_document.assert_called_once_with("", {"document_id": "empty-doc"})
